...
class SetCriterion(nn.Module):
    ...
    # def loss_boxes(self, outputs, target, indices, num_masks, mask_type):
    #     assert "pred_boxes" in outputs
    #     src_boxes = outputs["pred_boxes"].float()
    #
    #     losses = {"loss_boxes":}

    def loss_boxes(self, outputs, targets, indices, num_boxes, mask_type):
        ...
        #     map = outputs["pred_boxes"][batch_id][:, map_id].T
        #     target_bbox = target_box[batch_id][target_id]

        idx = self._get_src_permutation_idx(indices)
        target_boxes = torch.cat([t['boxes'][i] for t, (_, i) in zip(targets, indices)], dim=0).to('cuda')
    ...
    def forward(self, outputs, targets, mask_type, target_bbox=None):
