...
class InstanceSegmentation(pl.LightningModule):
    def __init__(self, config):
        ...
        weight_dict = {"loss_ce": matcher.cost_class,
                       ...
                       "loss_bbox": matcher.cost_box,
                       "loss_giou": matcher.cost_giou
                       }
    ...
    def forward(self, x, dn_data=None, point2segment=None, raw_coordinates=None, is_eval=False):
        with self.optional_freeze():
            x = self.model(x, dn_data, point2segment, raw_coordinates=raw_coordinates,
                           is_eval=is_eval)
        return x


    # def forward(self, x, bbox_data, point2segment=None, raw_coordinates=None, is_eval=False):
    #     with self.optional_freeze():
    #         x = self.model(x, bbox_data, point2segment, raw_coordinates=raw_coordinates,
    #                        is_eval=is_eval)
    #     return x
    ...
    def training_step(self, batch, batch_idx):
        ...
        if len(target) == 3:
            a = target[0]['masks'].shape[1]
            b = a + target[1]['masks'].shape[1]
            raw_coord_list = torch.tensor_split(raw_coordinates, [a, b], dim=0)
        else :
            raw_coord_list = (raw_coordinates, )
            # raw_coord_list = raw_coord_list.append(raw_coordinates)
        ##############BBOX DATA FROM GT MASKS################## NumPy
        # bbox_datas = []
        # for i in range(len(target)):
        #     label_data = []
        #     bbox_data = []
        #     for obj_id in range(target[i]['masks'].shape[0]):
        #         if target[i]['labels'][obj_id].item() == 255:
        #             continue
        #         # obj_coords = raw_coordinates[target[0]['masks'][obj_id, :].cpu().detach().numpy().astype(bool), :]
        #         # test = target[i]['masks'][obj_id, :].cpu().detach().numpy().astype(bool)
        #         obj_coords = raw_coord_list[i][target[i]['masks'][obj_id, :].cpu().detach().numpy().astype(bool), :]
        #         obj_coords = obj_coords.numpy()
        ...
        ##############BBOX DATA FROM GT MASKS##################

        ##############Processing DeNoising Datas##################
        # label_datas = []
        # bbox_datas = []
        # mask_datas = []
        # gt_boxes = []
        ...
        for i in range(len(target)):
            ...
            for obj_id in range(target[i]['masks'].shape[0]):
                if target[i]['labels'][obj_id].item() == 255:  # 253? 255? 0822 SONGOH
                    # print('check')
                    continue
                obj_coords = raw_coord_list[i][target[i]['masks'][obj_id, :].bool(), :]
                if obj_coords.shape[0] > 0:
                    obj_center = obj_coords.mean(dim=0)
                    obj_axis_length = obj_coords.max(dim=0).values - obj_coords.min(dim=0).values
                    bbox = torch.cat((obj_center, obj_axis_length))
                    bbox_data.append(bbox)
                    label_data.append(target[i]['labels'][obj_id])
                    mask_data.append(target[i]['masks'][obj_id])
            obj_dict['masks'] = torch.stack(mask_data).to('cuda')
            obj_dict['boxes'] = torch.stack(bbox_data).to('cuda')
            obj_dict['labels'] = torch.stack(label_data).to('cuda')
            dn_datas.append(obj_dict)
            target[i]['boxes'] = []
            target[i]['boxes'] = torch.stack(bbox_data, dim=0)
            # check = torch.stack(bbox_data).to('cuda')[0]
            # gt_boxes.append(obj_dict['boxes'])
        # self.bbox_gt = torch.stack(gt_boxes).to('cuda')
        # print('check')
        ##############Processing DeNoising Datas##################

        # for obj_id in range(target[0]['masks'].shape[0]):
        #     if target[0]['labels'][obj_id].item() == 255:
        #         continue
        #     obj_coords =
        ...
        try:
            output = self.forward(data,
                                  ...
                                  raw_coordinates=raw_coordinates, dn_data=dn_datas)
    ...
    def get_mask_and_scores(self, mask_cls, mask_pred, num_queries=100, num_classes=18, device=None):
        ...
        if self.config.general.topk_per_image != -1 :
            check1 = mask_cls.flatten(0, 1)
            check2 = check1.topk(self.config.general.topk_per_image, sorted = True)
